...
export async function GET(req: NextRequest) {
  ...
  const url = `https://example.com/genre/all?state=all&type=latest&filter=6&page=${page}`;
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch latest manga list." }, { status: 500 });
  }
}
